...
static func frames(p: Dictionary,exploded: float = 0) -> Array[Dictionary]:
	...
	var transforms := {"head":body*joint(p.head,Vector2(159,177)),"jaw":body*joint(p.jaw,Vector2(148,235)),"arm_l":left,"leg_l":left*joint(p.knee_l,Vector2(78,293)),"arm_r":right,"leg_r":right*joint(p.knee_r,Vector2(247,285)),"foot":body*joint(p.foot,Vector2(113,318))}
...
static func draw_details(canvas: CanvasItem,p: Dictionary,tex: Texture2D,tint: Color = Color.WHITE,root: Transform2D = Transform2D.IDENTITY) -> void:
	var body := Transform2D(p.body_tilt,p.body_offset)
	canvas.draw_set_transform_matrix(root*body*joint(p.head,Vector2(159,177))*Transform2D(0,-Vector2(155,200)))
	Eyes._eye(canvas,Vector2(247,175),8,p.gaze,tint,tex)
