...
func on_hit(ctx) -> void:
	if trigger_on_bounce:
		return
	_chain(ctx.pool, ctx.position, ctx.damage * damage_ratio, ctx.target)
...
func on_bounce(ctx) -> void:
	...
	_chain(pool, ctx.position, pool.get_damage_of(ctx.index) * damage_ratio, null)


func _chain(pool: ProjectilePool, from: Vector2, damage: float, exclude: Node) -> void:
	var hit: Array[Node] = []
	if exclude != null:
		hit.append(exclude)
	var origin := from
