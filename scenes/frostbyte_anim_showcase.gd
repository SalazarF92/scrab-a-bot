...
var _blend := 1.0
const LABELS := ["01  /  MOVIMENTO", "02  /  MORDIDA GLACIAL", "03  /  SOPRO DO FREEZER"]
const NOTES := ["Base de gelo • olhar e balanço", "Abertura • mordida • estilhaços", "Boca aberta • sopro congelante"]
const ACCENTS := [Color("#7DE8EE"), Color("#B2D9FF"), Color("#66C9FF")]

func _ready() -> void:
	_tex = load(Rig.ATLAS_PATH)
	_capture = "--capture-frost" in OS.get_cmdline_user_args()
	_exploded = "--explode-frost" in OS.get_cmdline_user_args()
...
func _process(delta: float) -> void:
	...
	if _capture:
		...
		if _frames in [2, 4, 6]:
			...
			var path := "res://docs/visual/frostbyte_%d.png" % (_frames / 2)
...
func _draw() -> void:
	...
	draw_string(font, Vector2(40, 78) * unit, "FROSTBYTE 500", HORIZONTAL_ALIGNMENT_LEFT, -1, int(32 * unit), Color("#E3F6FF"))
...
func _draw_card(rect: Rect2, action: int, pose: Dictionary, unit: float) -> void:
	...
	var sf := (.41 if _exploded else .65) * unit
	var floor_y := center.y + 213 * sf
