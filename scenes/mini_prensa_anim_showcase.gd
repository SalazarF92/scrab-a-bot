...
var _blend := 1.0
const LABELS := ["01  /  MOVIMENTO", "02  /  GOLPE HIDRÁULICO", "03  /  DESCARGA TÉRMICA"]
const NOTES := ["Passada alternada • suspensão mecânica", "Preparação • compressão • recuperação", "Carga • jato de plasma • resfriamento"]
const ACCENTS := [Color("#91D6C0"), Color("#FFBD69"), Color("#FF795C")]

func _ready() -> void:
	_tex = load(Rig.ATLAS_PATH)
	_capture = "--capture-press" in OS.get_cmdline_user_args()
	_exploded = "--explode-press" in OS.get_cmdline_user_args()
...
func _process(delta: float) -> void:
	...
	if _capture:
		...
		if _frames in [2, 4, 6]:
			...
			var path := "res://docs/visual/prensa_%d.png" % (_frames / 2)
...
func _draw() -> void:
	...
	draw_string(font, Vector2(40, 78) * unit, "MINI-PRENSA 500", HORIZONTAL_ALIGNMENT_LEFT, -1, int(32 * unit), Color("#F3E7D2"))
	draw_string(font, Vector2(vp.x - 370 * unit, 70 * unit), "PEÇAS RÍGIDAS • ESCALA FIXA", HORIZONTAL_ALIGNMENT_LEFT, -1, int(12 * unit), Color("#91D6C0"))
	if _mode == 3:
		for i in 3:
			var rect := Rect2(24 * unit + i * (vp.x - 48 * unit) / 3, 112 * unit, (vp.x - 48 * unit) / 3 - 12 * unit, vp.y - 200 * unit)
			var t := _time
			if _capture:
				t = [.24, .88, 1.3][mini(2, _frames / 2)]
	...
	draw_string(font, Vector2(32 * unit, vp.y - 22 * unit), status, HORIZONTAL_ALIGNMENT_LEFT, -1, int(11 * unit), Color("#91D6C0"))
...
func _draw_card(rect: Rect2, action: int, pose: Dictionary, unit: float) -> void:
	...
	var center := Vector2(rect.get_center().x, rect.position.y + 250 * unit)
	var sf := (.43 if _exploded else .70) * unit
	var floor_y := center.y + 223 * sf
