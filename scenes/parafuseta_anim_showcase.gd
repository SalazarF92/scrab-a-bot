...
var _blend := 1.0
const LABELS := ["01  /  MOVIMENTO", "02  /  BOTE MECÂNICO", "03  /  SOPRO DE PLASMA"]
const NOTES := ["Pernas articuladas • olhar ativo", "Preparação • bote • mordida", "Boca aberta • plasma • dissipação"]
const ACCENTS := [Color("#7DE8EE"), Color("#B2D9FF"), Color("#66C9FF")]

func _ready() -> void:
	texture_filter = CanvasItem.TEXTURE_FILTER_LINEAR_WITH_MIPMAPS
	_tex = load(Rig.ATLAS_PATH)
	_capture = "--capture-parafuseta" in OS.get_cmdline_user_args()
	_exploded = "--explode-parafuseta" in OS.get_cmdline_user_args()
...
func _process(delta: float) -> void:
	...
	if _capture:
		...
		if _frames in [2, 4, 6]:
			...
			var path := "res://docs/visual/parafuseta_%d.png" % (_frames / 2)
...
func _draw() -> void:
	...
	draw_string(font, Vector2(40, 78) * unit, "PARAFUSETA", HORIZONTAL_ALIGNMENT_LEFT, -1, int(32 * unit), Color("#E3F6FF"))
...
func _draw_card(rect: Rect2, action: int, pose: Dictionary, unit: float) -> void:
	...
	var sf := (.41 if _exploded else 1.05) * unit
