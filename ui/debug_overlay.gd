...
func _draw() -> void:
	...
	lines.append("inimigos %d" % get_tree().get_nodes_in_group(&"enemies").size())
	lines.append("")

	if robot != null and is_instance_valid(robot):
		lines.append("hp %.0f/%.0f   calor %.0f%%   watts %d/%d%s" % [
			robot.hp, robot.max_hp, robot.heat_ratio() * 100.0,
			robot.watts_used, robot.cpu.tdp if robot.cpu else 0,
	...
	lines.append("METAPROGRESSÃO: %d Cobre  |  Sucata atual: %d  |  Melhor Setor: %d" % [
		MetaManager.copper, MetaManager.current_scrap, MetaManager.best_sector
	])
	...
	if arena != null and not arena.last_report.is_empty():
		...
		lines.append("ARENA (GDD 3.3.4)")
		lines.append("  obstaculos %d (4 a 9)   cobertura %.0f%% (12 a 22%%) %s" % [
			r["obstacles"], r["coverage"] * 100.0, "ok" if r["coverage_ok"] else "FORA",
		])
		lines.append("  corredores >= 220 px: %s   alcancavel: %s" % [
			"ok" if r["corridors_ok"] else "FALHOU", "ok" if r["reachable_ok"] else "FALHOU",
		])
		lines.append("  ponto mais distante de superficie: %.0f px (teto 700) %s" % [
			r["max_surface_dist"], "ok" if r["surface_dist_ok"] else "FALHOU",
		])
		lines.append("")
	...
	lines.append("WASD mover   mouse mirar   clique E/D atirar   espaco dash   E purga")

	var y := 24.0
	var pad := Rect2(10, 10, 660, lines.size() * 19.0 + 14.0)
